...
confint_compare_graph<-function(p,n1,n2,conf.level, fix=2, step=.001) {
...
if (fix==1) {
	p1<-p;
	j<-seq(p1+step, .15, step);
	tab<-matrix(nrow=20,ncol=length(j))
	for (p2 in j) {
		tab[,i]<-as.vector(confint_compare(p1,n1,p2,n2,conf.level))
		i<-i+1;
		}
	}
else {
	p2<-p;
	j<-seq(.03,p2-step,step);
	tab<-matrix(nrow=20,ncol=length(j))
	for (p1 in j) {
		tab[,i]<-as.vector(confint_compare(p1,n1,p2,n2,conf.level))
		i<-i+1;
...
if (fix==1) {title(bquote(paste("PAVA: p2=prob.; ",p1==.(p),"; ",n1==.(n1),"; ",n2==.(n2),"; ",step==.(step))),outer=T)}
else {title(bquote(paste("PAVA: p1=prob.; ",p2==.(p),"; ",n1==.(n1),"; ",n2==.(n2),"; ",step==.(step))),outer=T)}
