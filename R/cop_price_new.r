...
#library(mvtnorm)  ## need mvtnorm
## change mc to be able to calc diff tranches without re-running sims

N_c <- 125;                ## number of credits in portfolio
...
t2 <- t[-1]
t3 <- t2 + t1
a <- 0.2	               ## credit correlation, could be vectorized
R <- 0.4	               ## recovery rate
...
p <- cond_def_prob(time,factor)  ## calc. conditional default prob outside loop
...
##  function nom_val takes tranche points and returns expected nominal value at each time of t
...
W <- W/runs
for (i in 1:(N_c+1)) nv[i] <- rn(i-1,al,ah)  ## calc nominal value remaining after each number of defaults
nom <- W%*%nv
return(spread(nom))
}
...
nom <- vector("numeric",length=length(t)) ## store nominal value for each time t_i for each run
for (i in 1:runs) {
d <- count2(rmvnorm(1,sigma=corr_mat,method="chol"))
for (j in 1:length(t)) W[j,d[j]+1] <- W[j,d[j]+1] + 1
 }
W <- W/runs
for (i in 1:(N_c+1)) nv[i] <- rn(i-1,al,ah)
...
#####  count function takes vector of default times and counts how many defaults have occured by each t_i
count <- function(x) {
y <- vector("numeric")
...
#########  The count2 function takes vector of gaussian variates and returns number of defaults by time t_i
count2 <- function(x) {
y <- vector("numeric")
