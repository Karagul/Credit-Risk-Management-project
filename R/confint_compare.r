...
### function confint_compare inputs probability of defaults for two classes (p1 and p2) and number of observations (n1 and n2) along with a confidence level.  
### Agresti-Coull, Wald, Score, Jefferys, and Zhou-Li two-sided confidence intervals are formed after constrained estimation of p1 and p2.  If p2 > p1, then the PAVA algorithm is used.
### The output is a matrix of coverage probability and expected length for each method

confint_compare <- function(p1,n1,p2,n2,conf.level){
...
for (x2 in 0:n2){											## Loop through each value of the number of defaults that are possible
  for (x1 in 0:n1) {
    if (x1/n1 <= x2/n2) {X1<-x1;X2<-x2;N1<-n1;N2<-n2} 				## Check to see if constraints on esimates are satisfied; if not -->PAVA
	  else {X1<-X2<-(x1+x2); N1<-N2<-(n1+n2)}
...
### note that in the case that PAVA is used to compute the estimate, the original number of observations is still used to form the width of the interval
...
if (N != n) {X = X*n/N}	### Check if N is not equal to n; if so, PAVA was used, so we adjust X so that it is smaller and gives the same estimate when divided by the original number of observations, n.  Since Beta dist. can take positive, non-integer values this is ok.  
if (X==0) {L<-0} else   
